...
class F1SeasonApp:
    ...
    def __init__(self):
        self.redbull_team: Optional[RedBullTeam] = None
    ...
    def register_team_for_season(self, team_name: str, budget: int):
        if team_name == 'Mercedes':
            ...
            self.redbull_team = RedBullTeam(budget)
        else:
    ...
    def new_race_results(self, race_name: str, red_bull_pos: int, mercedes_pos: int):
        if not self.redbull_team or not self.mercedes_team:
            raise Exception('Not all teams have registered for the season.')

        red_bull_revenue = self.redbull_team.calculate_revenue_after_race(red_bull_pos)
